...
func chapterSevenScene() -> World {
    ...
    let floorAndWallMaterial = Material(.solidColor(Color(1, 0.9, 0.9)), 0.1, 0.9, 0.0, 200)
    ...
    let leftBallMaterial = Material(.solidColor(Color(1, 0.8, 0.1)), 0.1, 0.7, 0.3, 200)
    let leftBall = Sphere(leftBallTransform, leftBallMaterial)


    let middleBallTransform = translation(-0.5, 1, 0.5)
    let middleBallMaterial = Material(.solidColor(Color(0.1, 1, 0.5)), 0.1, 0.7, 0.3, 200)
